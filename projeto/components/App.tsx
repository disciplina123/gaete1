...
import { Clock, BookOpen, BarChart3, Calendar as CalendarIcon, Settings as SettingsIcon, Trophy, Wrench, Music, ArrowLeft } from 'lucide-react';
import { Subject, Session, Book, Manga, EnemResult, FuvestResult, TabType, ColorMode, BackgroundMode, TaskGroup, TaskItem, SidebarLayout } from '../types';
...
const App: React.FC = () => {
  ...
  // --- Timer Logic ---
  const finishSession = useCallback((overrideQuestions?: number, overrideCorrect?: number, overrideDuration?: number) => {
  ...
  const completeSessionAndStartBreak = useCallback((questions?: number, correct?: number, duration?: number) => {
    finishSession(questions, correct, duration);
    setIsBreak(true);
  ...
  const addArea = (subjectId: number, name: string) => {
  ...
  const deleteArea = (subjectId: number, areaId: number) => {
    const newSubjects = subjects.map(subject => {
      if (subject.id === subjectId) {
        return {
          ...subject,
          areas: subject.areas.filter(a => a.id !== areaId)
        };
      }
      return subject;
    });
  ...
  const addChapter = (subjectId: number, areaId: number, name: string) => {
    const newSubjects = subjects.map(subject => {
      if (subject.id === subjectId) {
        return {
          ...subject,
          areas: subject.areas.map(area => {
            if (area.id === areaId) {
              return {
                ...area,
  ...
  const deleteChapter = (subjectId: number, areaId: number, chapterId: number) => {
    const newSubjects = subjects.map(subject => {
      if (subject.id === subjectId) {
        return {
          ...subject,
          areas: subject.areas.map(area => {
            if (area.id === areaId) {
              return {
                ...area,
                chapters: area.chapters.filter(c => c.id !== chapterId)
              };
            }
            return area;
          })
  ...
  const addTask = (subjectId: number, areaId: number, chapterId: number, name: string) => {
    const newSubjects = subjects.map(subject => {
      if (subject.id === subjectId) {
        return {
          ...subject,
          areas: subject.areas.map(area => {
            if (area.id === areaId) {
              return {
                ...area,
                chapters: area.chapters.map(chapter => {
  ...
  const toggleTask = (subjectId: number, areaId: number, chapterId: number, taskId: number) => {
    const newSubjects = subjects.map(subject => {
      if (subject.id === subjectId) {
        return {
          ...subject,
  ...
  const deleteTask = (subjectId: number, areaId: number, chapterId: number, taskId: number) => {
    const newSubjects = subjects.map(subject => {
      if (subject.id === subjectId) {
        return {
          ...subject,
  ...
  // NOTE: Renaming logic simplified for brevity but follows same pattern as create/delete
  const updateAreaName = (subjectId: number, areaId: number, newName: string) => {
    const newSubjects = subjects.map(subject => {
      if (subject.id === subjectId) {
        return {
          ...subject,
  ...
  const updateChapterName = (subjectId: number, areaId: number, chapterId: number, newName: string) => {
    const newSubjects = subjects.map(subject => {
      if (subject.id === subjectId) {
        return {
          ...subject,
  ...
  const updateTaskName = (subjectId: number, areaId: number, chapterId: number, taskId: number, newName: string) => {
    const newSubjects = subjects.map(subject => {
      if (subject.id === subjectId) {
        return {
          ...subject,
      ...
      <div className={`tabs ${sidebarLayout === 'gaete' ? 'tabs-gaete' : ''} ${sidebarLayout === 'bottom' ? 'tabs-bottom' : ''}`}>
        ...
        <div className="sidebar-nav">
          {[
            ...
            { id: 'library', icon: Wrench, title: 'Ferramentas' },
      ...
      <div className="content tab-entering" ref={contentRef}>
        ...
        {activeTab === 'library' && (
          <Tools 
            ...
            onUpdateTaskGroupName={handleUpdateTaskGroupName}
